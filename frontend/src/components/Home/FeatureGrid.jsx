...
export function FeatureGrid() {
  return (
    <section className="py-12 px-6 sm:px-8 max-w-[1200px] mx-auto w-full relative z-10 font-mono">
      
      {/* Section Header */}
      <div className="mb-10 md:mb-12 mt-8 md:mt-12">
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-white mb-4">
          Think fast. <br className="hidden sm:block" />
          <span className="text-gray-400">Connect faster.</span>
        </h2>
        ...
        {features.map((feature, idx) => {
          ...
          return (
            <div 
              key={idx}
              className={cn(
                "group relative overflow-hidden rounded-xl transition-all duration-300 hover:-translate-y-1 flex flex-col border border-white/5 bg-white/[0.02]",
                feature.className
              )}
            >
              ...
              <div className="relative z-10 flex flex-col h-full w-full p-5 md:p-6">
                <div className="relative z-10 h-full flex flex-col">
                  <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-white/5 border border-white/10 text-gray-400 group-hover:text-white transition-colors">
                  ...
                  <p className="text-sm leading-relaxed text-gray-500 group-hover:text-gray-400 transition-colors flex-grow">
                    {feature.description}
                  </p>

                  {/* Optional visual placeholder for illustrations */}
                  <div className="mt-6 pt-5 border-t border-white/5 flex items-center justify-between">
                      <span className="text-xs font-medium text-gray-500 group-hover:text-white transition-colors uppercase tracking-wider">Explore Feature</span>
                      <svg className="h-4 w-4 text-gray-600 group-hover:text-white transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
